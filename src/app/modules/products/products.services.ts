...
const getAllProduct = async (
  ...
  const { minPrice, maxPrice, categories, ...pQuery } = query;
  ...
  if (authUser) {
    ...
    const wishlistProductIds = new Set(
      wishlist.map((item) => item.product.toString())
    );
...
const getAllProductByAdmin = async (query: Record<string, unknown>) => {
  const { minPrice, maxPrice, categories, ...pQuery } = query;
  const productQuery = new QueryBuilder(
    Product.find().populate("userId", "name phoneNumber"),
    pQuery
  )
    .search(["title", "description"])
    .filter()
...
const getAllUserProduct = async (
  ...
  const { minPrice, maxPrice, categories, ...pQuery } = query;
  const productQuery = new QueryBuilder(
    Product.find({ userId: userID }).populate("userId", "name phoneNumber"),
    pQuery
  )
    .search(["title", "description"])
    .filter()
...
const deleteProduct = async (productId: string) => {
  ...
  try {
    ...
    const wishlist = await Wishlist.deleteMany({ product: productId }).session(
      session
    );
    if (!wishlist) {
      throw new AppError(
